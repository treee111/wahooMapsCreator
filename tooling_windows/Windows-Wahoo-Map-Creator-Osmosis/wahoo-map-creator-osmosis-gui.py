...
# Tags to keep
filtered_tags = 'access= admin_level= aerialway= aeroway= barrier= boundary= bridge= highway= natural= oneway= place= ' \
...
africa = ['algeria', 'angola', 'benin', 'botswana', 'burkina_faso', 'burundi', 'cameroon', 'cape_verde',
          'central_african_republic', 'chad', 'comoros', 'cote_d_ivoire', 'democratic_republic_of_the_congo',
          'djibouti', 'egypt', 'equatorial_guinea', 'eritrea', 'ethiopia', 'french_southern_territories', 'gabon',
          'gambia', 'ghana', 'guinea-bissau', 'guinea', 'kenya', 'lesotho', 'liberia', 'libya', 'madagascar', 'malawi',
          'mali', 'mauritania', 'mauritius', 'mayotte', 'morocco', 'mozambique', 'namibia', 'niger', 'nigeria',
          'republic_of_congo', 'reunion', 'rwanda', 'saint_helena', 'sao_tome_and_principe', 'senegal', 'seychelles',
          'sierra_leone', 'somalia', 'south_africa', 'sudan', 'swaziland', 'tanzania', 'togo', 'tunisia', 'uganda',
...
africa_geofabrik = ['algeria', 'angola', 'benin', 'botswana', 'burkina-faso', 'burundi', 'cameroon', 'Canary Islands',
                    'cape-verde', 'central african republic', 'chad', 'comores', 'Congo (Republic/Brazzaville)',
                    'congo-democratic-republic', 'djibouti', 'egypt', 'Equatorial Guinea', 'eritrea', 'ethiopia',
...
asia_geofabrik = ['afghanistan', 'armenia', 'azerbaijan', 'bangladesh', 'bhutan', 'cambodia', 'china', 'gcc-states',
                  'india', 'indonesia', 'iran', 'iraq', 'israel-and-palestine', 'japan', 'jordan', 'kazakhstan',
                  'kyrgyzstan', 'laos', 'lebanon', 'malaysia-singapore-brunei', 'maldives', 'mongolia', 'myanmar',
                  'nepal', 'north-korea', 'pakistan', 'philippines', 'russian federation', 'south-korea', 'sri-lanka',
                  'syria', 'taiwan', 'tajikistan', 'thailand', 'turkmenistan', 'uzbekistan', 'vietnam', 'yemen']

australiaoceania_geofabrik = ['american oceania', 'australia', 'cook islands', 'fiji', 'île de clipperton', 'kiribati',
                              'marshall islands', 'micronesia', 'nauru', 'new caledonia', 'new zealand', 'niue',
                              'palau', 'papua new guinea', 'pitcairn islands', 'polynesie-francaise', 'samoa',
                              'solomon islands', 'tokelau', 'tonga', 'tuvalu', 'vanuatu', 'wallis et futuna']
...
europe_geofabrik = ['albania', 'andorra', 'austria', 'azores', 'belarus', 'belgium', 'bosnia-herzegovina',
                    ...
                    'ireland-and-northern-ireland', 'isle-of-man', 'italy', 'kosovo', 'latvia', 'liechtenstein',
                    'lithuania', 'luxembourg', 'macedonia', 'malta', 'moldova', 'monaco', 'montenegro', 'netherlands',
                    'norway', 'poland', 'portugal', 'romania', 'russian federation', 'serbia', 'slovakia', 'slovenia',
                    'spain', 'sweden', 'switzerland', 'turkey', 'ukraine (with crimea)']
...
germany_subregions_geofabrik = ['baden-württemberg', 'bayern', 'berlin', 'brandenburg (mit berlin)', 'bremen',
...
france_subregions_geofabrik = ['alsace', 'aquitaine', 'auvergne', 'basse-normandie', 'bourgogne', 'bretagne', 'centre',
                               'champagne ardenne', 'corse', 'franche comte', 'guadeloupe', 'guyane', 'haute-normandie',
                               'ile-de-france', 'languedoc-roussillon', 'limousin', 'lorraine', 'martinique', 'mayotte',
...
cboContinent = ttk.Combobox(app, values=continents, state="readonly", width=15)
cboCountry = ttk.Combobox(app, values=europe, state="readonly", width=15)
...
region = ''
if country_gui in africa:
    region = 'africa'
if country_gui in antarctica:
    region = 'antarctica'
if country_gui in asia:
    region = 'asia'
if country_gui in europe:
    region = 'europe'
if country_gui in northamerica:
    region = 'north-america'
if country_gui in oceania:
    region = 'oceania'
if country_gui in southamerica:
    region = 'south-america'
if country_gui in unitedstates:
...
print('\n\n# read json file')
with open(os.path.join('json', region, country_gui + '.json')) as f:
    country = json.load(f)
f.close()
if country == '':
...
now = time.time()
Too_Old = now - 60 * 60 * 24 * Max_Days_Old
try:
    FileCreation = os.path.getctime(land_polygons_file)
    if FileCreation < Too_Old:
...
for c in border_countries:
    ...
    if len(map_files) == 1 and os.path.isfile(map_files[0]):
        ...
        if FileCreation < Too_Old or Force_Processing == 1:
...
for tile in country:
    ...
    # search for user entered country name in translated (to geofabrik).
    # if match continue with matched else continue with user entered country search
    # for country match in geofabrik tables to determine region to use for map download
    for c in tile['countries']:
        if c not in border_countries:
            ...
            if len(map_files) != 1 or not os.path.isfile(map_files[0]):
                ...
                try:
                    ...
                    if region != 'no':
                        url = 'https://download.geofabrik.de/' + region + '/' + Translate_Country[
                            f'{c}'] + '-latest.osm.pbf'
                    else:
...
for key, val in border_countries.items():
    ...
    if not os.path.isfile(outFile) or Force_Processing == 1:
        ...
        cmd = ['osmconvert']
        cmd.extend(['-v', '--hash-memory=2500', '--complete-ways', '--complete-multipolygons', '--complete-boundaries',
...
for tile in country:
    for c in tile['countries']:
        ...
        if not os.path.isfile(outFile) or Force_Processing == 1:
            ...
            cmd.append(
                '-b=' + f'{tile["left"]}' + ',' + f'{tile["bottom"]}' + ',' + f'{tile["right"]}' + ',' + f'{tile["top"]}')
...
for tile in country:
    ...
    if not os.path.isfile(outFile) or Force_Processing == 1:
        ...
        for land in land_files:
            cmd.extend(
                ['--rx', 'file=' + os.path.join(OUT_PATH, f'{tile["x"]}', f'{tile["y"]}', f'{land}'), '--s', '--m'])
        cmd.extend(['--rx', 'file=' + os.path.join(OUT_PATH, f'{tile["x"]}', f'{tile["y"]}', f'sea.osm'), '--s', '--m'])
        cmd.extend(['--tag-transform', 'file=' + os.path.join(CurDir, 'tunnel-transform.xml'), '--wb', outFile,
                    'omitmetadata=true'])
        # print(cmd)
        result = subprocess.run(cmd)
        if result.returncode != 0:
...
for tile in country:
    ...
    if not os.path.isfile(outFile + '.lzma') or Force_Processing == 1:
        ...
        cmd = [os.path.join(CurDir, 'Osmosis', 'bin', 'osmosis.bat'), '--rbf', mergedFile, 'workers=' + workers, '--mw',
               'file=' + outFile]
...
countryName = os.path.split(country_gui)
print(countryName[1])
# Make Wahoo zip file
